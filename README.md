...
API 接口具备强大的参数配置，支持所有参数的自定义设置，同时提供最丰富的滤波器种类选择，能够满足复杂场景下的精准处理需求。但是，由于参数存在功能关联，不同参数的配置组合容易引发冲突，导致异常。

**SDK 接口说明**

SDK 接口的使用简洁直观，可快速配置与部署。缺点是可配置的参数范围有限，仅支持基础调节，无法实现精细化的参数定制
...
```
**原理**: 配置硬件资源，提升性能
- **hardware_shaves**: 分配的SHAVE处理器数量
- **hardware_memory_slices**: 分配的内存切片数量 
